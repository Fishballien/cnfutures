...
def visualize_factor_returns(ind_cate_name, final_path_name, factor_data_dir, batch_test_name, tag_name,
    ...
    Each org_factor x test_name gets its own figure with subplots for each final_path.
    ...
    # Get period name for saving files
    ...
    save_dir = Path(path_config['result']) / 'batch_path_factor_visualization' / ind_cate_name / final_path_name
    save_dir.mkdir(exist_ok=True, parents=True)
    
    # First, identify all original factors (org_factors) from the factor files
    all_org_factors = set()
    for process_name in process_name_list:
        factor_path = Path(factor_data_dir) / process_name
        try:
            for path in factor_path.glob('*.parquet'):
                # Extract org_factor from the file name (similar to select_basic_features.py)
                factor_name = path.stem
                org_factor = factor_name.split('-', 1)[0]  # Split on first hyphen
                all_org_factors.add(org_factor)
        except Exception as e:
            print(f"Error getting org_factors from {process_name}: {str(e)}")
    
    print(f"Found {len(all_org_factors)} original factors: {', '.join(sorted(all_org_factors))}")
    
    # Group by org_factor and test_name
    for org_factor in sorted(all_org_factors):
        for mode, test_name in test_name_list:
            print(f"Processing org_factor: {org_factor}, test: {test_name}")
            
            # Filter the process infos for the current test name
            process_infos = [info for info in process_info_list if info[4] == test_name]
            
            # Extract unique process names for this org_factor
            unique_process_names = []
            for process_name in process_name_list:
                factor_path = Path(factor_data_dir) / process_name
                has_matching_factors = False
                try:
                    for path in factor_path.glob(f"{org_factor}-*.parquet"):
                        has_matching_factors = True
                        break
                    if has_matching_factors:
                        unique_process_names.append(process_name)
                except Exception as e:
                    print(f"Error checking factors for {process_name}: {str(e)}")
            
            # Skip if no process names found for this org_factor
            if not unique_process_names:
                print(f"No matching factors found for org_factor: {org_factor}")
                continue
            ...
            fig.suptitle(f"{org_factor} - Cumulative {return_type.upper()} Returns ({direction_type}) - {test_name}", 
                         fontsize=16)
            ...
            for i, process_name in enumerate(unique_process_names):
                ...
                # Get matching factor list for this org_factor
                factor_path = Path(factor_data_dir) / process_name
                try:
                    # Filter factors that match the current org_factor prefix
                    factor_name_list = [path.stem for path in factor_path.glob(f"{org_factor}-*.parquet")]
                    
                    if not factor_name_list:
                        print(f"No matching factors found for {org_factor} in {process_name}")
                        continue
                    ...
                    # Add a label with the number of factors in the LEFT top corner
# =============================================================================
#                     ax.text(0.05, 0.95, 
...
# =============================================================================
                    ...
                    # Add final cumulative return in the RIGHT top corner
                    final_mean_return = cum_mean_returns.iloc[-1] if not cum_mean_returns.empty else 0
                    ax.text(0.95, 0.95, 
                            f"Final Return: {final_mean_return:.2%}", 
                            transform=ax.transAxes, 
                            verticalalignment='top',
            ...
            save_path = save_dir / f"factor_returns_{org_factor}_{tag_name}_{final_path_name}_{test_name}_{return_type}_{direction_type}_{period_name}.png"
...
if __name__ == "__main__":
    visualize_factor_returns(
        ...
        final_path_name="org_TS_oad_0931_batch_250419",
