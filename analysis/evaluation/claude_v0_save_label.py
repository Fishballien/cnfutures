...
def generate_time_chunks(price_data, chunk_size_days=30):
    """
    Generate time-based chunks for processing.
    
    Parameters:
    - price_data: DataFrame with datetime index
    - chunk_size_days: Approximate chunk size in days
    
    Returns:
    - chunks: List of dates defining chunk boundaries
    """
    dates = pd.Series(price_data.index).dt.date.unique()
    
    # Generate chunks of approximately chunk_size_days
    chunks = [dates[0]]  # Start with the first date
    
    for i in range(1, len(dates), chunk_size_days):
        if i < len(dates):
            chunks.append(dates[i])
    
    # Add the end date if not already included
    if chunks[-1] != dates[-1]:
        chunks.append(dates[-1])
...
if __name__ == "__main__":
    ...
    time_chunk_size = 30  # Days per time chunk
    ...
    time_chunks = generate_time_chunks(price_data, chunk_size_days=time_chunk_size)
    print(f"Processing in {len(time_chunks)-1} time chunks of approximately {time_chunk_size} days each")
    ...
    # Plot results for a specific asset (e.g., 'IC') - using just the last chunk for visualization
