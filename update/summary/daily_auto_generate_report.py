...
from update.summary.generate_report_with_mail import generate_report
...
def update_tecm(update_name=None, delay=1):
    ...
    # 读取参数
    with open(param_dir / 'daily_report' / f'{update_name}.yaml', "r") as file:
    ...
    # 更新近10-20天股票ind
    rp_params = params['report']
    rp_params['target_date'] = datetime.strptime(target_date, '%Y%m%d').strftime('%Y-%m-%d')
    output = params['output']
    dependency = params['dependency']
    
    with coordinator(output, dependency):
        if not coordinator.skip_task:
            generate_report(**rp_params)
