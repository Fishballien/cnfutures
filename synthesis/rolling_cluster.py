...
class RollingCluster:
    ...
    def __init__(self, cluster_name, pstart='20230701', 
                 puntil=None, cluster_type='rolling', check_consistency=True,
    ...
    def _init_dir(self):
        ...
        self.param_dir = self.param_dir / self.base_cluster_name if self.base_cluster_name else self.param_dir
    ...
    def run(self):
        offset_days = self.params.get('offset_days', 0)
        rolling_dates = self._get_rolling_dates()
        long_periods, rec_periods, super_rec_periods = self._get_all_fit_periods(rolling_dates)
        
        cl = Cluster(self.cluster_name, check_consistency=self.check_consistency,
                     base_cluster_name=self.base_cluster_name)
        ...
        if self.cluster_type == 'rolling':
            ...
            for fp, rp, srp in tqdm(list(periods_zip), desc='clustering'):
                ...
                if rp is not None:
                    rp = list(rp)  # Convert tuple to list
                    rp[1] -= timedelta(days=offset_days)  # Modify the second element
                if srp is not None:
                    srp = list(srp)  # Convert tuple to list
                    srp[1] -= timedelta(days=offset_days)  # Modify the second element
