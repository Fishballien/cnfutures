...
class FactorMerger:
    ...
    def __init__(self, merge_name, max_workers=None):
        ...
        self.param_dir = Path(self.path_config['param']) / 'merge_selected_ts_trans'
        self.test_dir = self.result_dir / 'test'
        self.merged_dir = self.result_dir / 'merge_selected_ts_trans' / merge_name
        
        # 加载配置文件
        config_path = self.param_dir / f'{merge_name}.yaml'
        self.config = self._load_config(config_path)
        
        self.select_dir = self.result_dir / 'select_ts_trans' / self.config['select_name']
