...
from utils.datautils import (get_one_factor, load_all_factors, load_one_group, align_index_with_main,
...
# %% funcs
def model_predict(model, X, y, y_data, to_mask, to_mask_ravel):
...
class ModelFit(ABC):
    ...
    def __init__(self, model, test_name, preprocess_params={}, fit_params={}, predict_params={},
                 n_workers=1, check_consistency=True, base_model_name=None):
        self.model = model
    ...
    def _load_public_paths(self):
        ...
        price_type = self.preprocess_params.get('price_type', 'future_twap')
        
        self.result_root_dir = Path(self.path_config['result'])
        self.data_dir = Path(self.path_config['factor_data'])
        self.twap_dir = Path(self.path_config[price_type])
        self.cluster_dir = self.result_root_dir / 'cluster'
        
    def _init_dir(self):
        self.result_dir = (self.result_root_dir / 'model' / self.test_name
                           if self.base_model_name is None
                           else self.result_root_dir / 'model' / self.base_model_name / self.test_name)
    ...
    def _init_utils(self):
        self.normalization_func = partial(ts_normalize, param=self.preprocess_params)
    ...
    def _load_n_preprocess_twap_return(self, start_date, end_date):
        ...
        symbols = self.preprocess_params.get('symbols')
        pred_wd = self.preprocess_params.get('pred_wd')
        sp = self.preprocess_params['sp']
        
        twap_path = self.twap_dir / f'{twap_name}.parquet'
        twap_price = pd.read_parquet(twap_path)
        twap_price = twap_price[symbols] if symbols is not None else twap_price
        ...
        self.to_mask = (rtn_1p.isna() if self.mode == 'fit' 
                        else pd.DataFrame(False, index=rtn_1p.index, columns=rtn_1p.columns))
    ...
    def _load_n_preprocess_factors(self, start_date, end_date, ref_order_col, ref_index):
        ...
        base_cluster = self.preprocess_params.get('base_cluster')
        price_type = self.preprocess_params.get('price_type', 'future_twap')
        fix_changed_root = self.preprocess_params.get('fix_changed_root', False)

        if cluster is None:
            raise NotImplementedError()
        cluster_file_dir = self.cluster_dir / base_cluster / cluster
        ...
        get_one_factor_func = partial(get_one_factor, price_type=price_type,
        ...
        load_one_group_func = partial(load_one_group, to_mask=self.to_mask)
    ...
    def check_exists(self, model_start_date, model_end_date, 
                     pred_start_date, pred_end_date, source='multi'): # HINT: 暂时不用
        model_source_prefix = f'{source}_'
        model_period_suffix = period_shortcut(model_start_date, model_end_date)
        model_suffix = f'{model_source_prefix}{model_period_suffix}'
        predict_suffix = period_shortcut(pred_start_date, pred_end_date)
        pred_path = self.predict_dir / f'predict_{predict_suffix}_by_{model_suffix}.parquet'
    ...
    def _save_predict_and_merge(self, y_pred):
        # if self.predict_suffix.startswith('250101'):
        #     breakpoint()
        # 保存当前预测结果到单独的文件
...
# =============================================================================
    ...
    def _save_predictions(self, y_pred):
        """
        将新的预测结果添加到已有的汇总预测文件中，如果debug_dir存在且已有相同文件，则先检查一致性
        ...
        if os.path.exists(pred_all_path):
            ...
            if self.check_consistency and hasattr(self, 'debug_dir'):
                try:
                    # 直接使用check_dataframe_consistency函数检查一致性
                    status, info = check_dataframe_consistency(pred_all, y_pred)
                    
                    if status == "INCONSISTENT":
                        # 保存不一致的数据到debug目录
                        debug_path = self.debug_dir / f'predict_{self.test_name}_inconsistent.parquet'
...
class AverageModelFit(ModelFit):
    ...
    def _model_predict(self, model, X, y, y_data):
        score = model.score(X, y)
        self.log.info(f'oos score: {score}')
        y_pred = model_predict(model, X, y, y_data, 
                               self.to_mask, self.to_mask_ravel)
        self._save_predict_and_merge(y_pred)
