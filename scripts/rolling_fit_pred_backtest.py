...
def main(test_name=None, puntil=None, fstart='20210101', pstart='20220101', mode='rolling', n_workers=1,
         check_consistency=True, base_model_name=None):
    ...
    param_dir = param_dir / base_model_name if base_model_name else param_dir
    ...
    mf = model_fit(test_name, preprocess_params=preprocess_params, fit_params=fit_params, predict_params=predict_params,
                   n_workers=n_workers, check_consistency=check_consistency, base_model_name=base_model_name,
                   )
    ...
    offset_days = rolling_params_variable.get('offset_days', 0)
    
    # run fit & predict
    if mode == 'rolling':
        mf.test_predicted()
        for i, (fltp, fp, pp) in enumerate(list(zip(filter_periods, fit_periods, predict_periods))):
            fp = list(fp)  # Convert tuple to list
            fp[1] -= timedelta(days=offset_days)  # Modify the second element
            fltp = list(fltp)  # Convert tuple to list
            fltp[1] -= timedelta(days=offset_days)  # Modify the second element
