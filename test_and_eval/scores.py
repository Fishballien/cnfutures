...
def calc_return_annualized(return_arr):
    return np.nansum(return_arr) * ONEYEAR_TRADING_DAYS / len(return_arr)
...
def calc_sharpe(return_arr):
    if return_std := np.std(return_arr):
        return np.mean(return_arr) / return_std * np.sqrt(ONEYEAR_TRADING_DAYS)
    else:
        return np.nan
    
    
def calc_calmar_ratio(return_arr):
    ret_annualized = np.nansum(return_arr) * ONEYEAR_TRADING_DAYS / len(return_arr)
...
def calculate_sortino_ratio(returns, risk_free_rate=0):
    ...
    rf_daily = (1 + risk_free_rate) ** (1 / ONEYEAR_TRADING_DAYS) - 1
    ...
    return mean_excess_return / downside_std * np.sqrt(ONEYEAR_TRADING_DAYS)
...
def calculate_sterling_ratio(returns, annual_target=0):
    ...
    annual_return = (1 + cumulative_return)**(ONEYEAR_TRADING_DAYS / len(returns)) - 1
...
def get_general_return_metrics(return_arr):
    ...
    ret_annualized = np.sum(return_arr) * ONEYEAR_TRADING_DAYS / len(return_arr)
    max_dd = calc_max_drawdown(return_arr)
